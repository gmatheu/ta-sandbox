...
class Watchlist(object):
    ...
    def _load_all(self, force=False, cache=True, **kwargs) -> dict:
        ...
        if (self.tickers is not None and isinstance(self.tickers, list) and
                len(self.tickers)):
            self.data = {ticker: self.load(ticker, force=force, cache=cache, **kwargs) for ticker in self.tickers}
            return self.data
    ...
    def load(self, ticker: str = None, tf: str = None, index: str = "date", drop: list = [],
        ...
        if ticker is not None and isinstance(ticker, str):
            ...
            self._load_all(force=force, cache=cache, **kwargs)
            return
        ...
        if current_file.exists() and not force and cache:
            # if self.ds_name == "av":
        ...
        timeframe_loaded = f"[i] Loaded dates {ticker}[{tf}]: from {first_timestamp} to {last_timestamp}"
        print(timeframe_loaded)
